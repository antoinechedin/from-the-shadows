...
public class SwingMovement : MonoBehaviour, IResetable
{
    ...
    void Start()
    {
        ...
        Transform child = swingingObject.transform.Find("Child");
        if(child != null)
            child.Translate(new Vector3(0, -ropeLength, 0));

        if (swingingObject.GetComponentInChildren<Torch>() != null) 
            swingingObject.GetComponentInChildren<Torch>().neverStatic = true;
    }
